...
# # _ and r
# melody_class_weights[melody_mappings_indexes['_']] = 0.325  # 0.3 - 0.35 
# melody_class_weights[melody_mappings_indexes['r']] = 1.25

# # notes
# melody_class_weights[melody_mappings_indexes['64']] = 1.5
# melody_class_weights[melody_mappings_indexes['65']] = 2.95
# melody_class_weights[melody_mappings_indexes['67']] = 2.6 
...
melody_optimizer = torch.optim.Adam(melody_model.parameters(), lr=0.0001)
chords_optimizer = torch.optim.Adam(chords_model.parameters(), lr=0.0001)
...
for epoch in range(num_epochs):
    ...
    if not os.path.exists(DEFAULT_MODEL_WEIGHTS_FOLDER_NAME(idx = epoch)):
        os.makedirs(DEFAULT_MODEL_WEIGHTS_FOLDER_NAME(idx = epoch))
        
    torch.save(chords_model.state_dict(), DEFAULT_CHORDS_MODEL_WEIGHTS_FILE_NAME(idx = epoch))
    torch.save(melody_model.state_dict(), DEFAULT_MELODY_MODEL_WEIGHTS_FILE_NAME(idx = epoch))
